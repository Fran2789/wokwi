...
class SmartHomeBackend:
    ...
    def apply_auto_control(self, temperature, humidity, _light):
     ...
     # ======================================
     #    CONTROL DE LUZ POR HORARIO LOCAL
     # ======================================
     now = datetime.now().time()
 
     start = datetime.strptime("19:00", "%H:%M").time()   # 7 PM
     end   = datetime.strptime("17:00", "%H:%M").time()   # 5 PM
 
     # Rango que cruza medianoche:
     # ON si hora >= 19:00 o <= 17:00
     is_light_time = (now >= start) or (now <= end)
 
     if is_light_time:
         if not self.actuator_states["light"]:
             self.actuator_states["light"] = True
             commands["light"] = "on"
             self.database.save_actuator_event("light", "on", auto_triggered=True)
             print("   🌙 Auto-Horario: Luz ON (entre 7 PM y 5 PM)")
     else:
         if self.actuator_states["light"]:
             self.actuator_states["light"] = False
             commands["light"] = "off"
             self.database.save_actuator_event("light", "off", auto_triggered=True)
             print("   ☀️ Auto-Horario: Luz OFF (fuera de horario)")
